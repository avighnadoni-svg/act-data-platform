-- ============================================================================
-- ACT Data Platform - Process VISIT Option 3
-- File: snowflake/sql/018_process_visit_option3.sql
--
-- Flow:
--
--   S3
...
-- Replay protection:
--   HISTORY uniqueness =
--       STUDY_ID + VISIT_ID + UPDATED_AT + RECORD_HASH
--
-- Current-state rule:
--   Most recently received version wins.
--
-- Normalized VISIT CSV column order:
--
--   1  visit_id
--   2  study_id
--   3  subject_id
...
-- ============================================================================
-- 1. COPY NEW VISIT FILES INTO LANDING
-- ============================================================================

COPY INTO ACT_DB.RAW.LND_VISIT
(
    VISIT_ID,
    STUDY_ID,
    SUBJECT_ID,
...
(
    SELECT
        ...
        TRY_TO_TIMESTAMP_TZ(t.$7::VARCHAR),
...
SET RECORD_HASH =
    SHA2(
        CONCAT_WS(
            ...
            COALESCE(TO_VARCHAR(PLANNED_DATE, 'YYYY-MM-DD'), '<NULL>'),
            COALESCE(TO_VARCHAR(ACTUAL_DATE, 'YYYY-MM-DD'), '<NULL>')
        ),
        256
    )
...
SELECT
    ...
    COUNT_IF(VISIT_ID IS NULL) AS NULL_VISIT_ID_ROWS,
...
MERGE INTO ACT_DB.RAW.RAW_VISIT_HISTORY AS T
USING
(
    SELECT
        VISIT_ID,
...
MERGE INTO ACT_DB.RAW.RAW_VISIT_CURRENT AS T
USING
(
    SELECT
        VISIT_ID,
...
THEN UPDATE SET
    ...
    T.VISIT_NAME = S.VISIT_NAME,
    T.PLANNED_DATE = S.PLANNED_DATE,
